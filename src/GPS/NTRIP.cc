    ...
    , _ntripStatus(tr("Disconnected"))
{
    ...
    if (qgcApp() && qgcApp()->toolbox()) {
        QGCToolbox* tb = qgcApp()->toolbox();

        // First, try to find an existing RTCMMavlink tool (if some other code created it)
        _rtcmMavlink = tb->findChild<RTCMMavlink*>();

        // If none exists, create one attached to the toolbox
        if (!_rtcmMavlink) {
            _rtcmMavlink = new RTCMMavlink(*tb);   // matches ctor RTCMMavlink(QGCToolbox& toolbox)
            _rtcmMavlink->setObjectName(QStringLiteral("RTCMMavlink"));
            qCDebug(NTRIPLog) << "NTRIP: Created RTCMMavlink helper tool";
        }
    } else {
        qCWarning(NTRIPLog) << "NTRIP: QGCApplication/toolbox not available; RTCMMavlink disabled";
    }

    // Force NTRIP checkbox OFF during app startup and keep it OFF until first settings tick.
    {
        NTRIPSettings* settings = nullptr;

        if (qgcApp() && qgcApp()->toolbox() && qgcApp()->toolbox()->settingsManager()) {
            SettingsManager* sm = qgcApp()->toolbox()->settingsManager();
            settings = sm->ntripSettings();
        }
...
void NTRIPManager::_rtcmDataReceived(const QByteArray& data)
{
    ...
    if (!_rtcmMavlink && qgcApp() && qgcApp()->toolbox()) {
        _rtcmMavlink = qgcApp()->toolbox()->findChild<RTCMMavlink*>();
    }
...
void NTRIPManager::_sendGGA()
{
    ...
    if (qgcApp()) {
        ...
        MultiVehicleManager* mvm = nullptr;
        if (qgcApp() && qgcApp()->toolbox()) {
            mvm = qgcApp()->toolbox()->multiVehicleManager();
        }
        if (mvm) {
        ...
        if (!validCoord) {
            MultiVehicleManager* mvm2 = nullptr;
            if (qgcApp() && qgcApp()->toolbox()) {
                mvm2 = qgcApp()->toolbox()->multiVehicleManager();
            }
            if (mvm2) {
...
void NTRIPManager::_checkSettings()
{
    ...
    NTRIPSettings* settings = nullptr;
    if (qgcApp() && qgcApp()->toolbox() && qgcApp()->toolbox()->settingsManager()) {
        settings = qgcApp()->toolbox()->settingsManager()->ntripSettings();
    }
